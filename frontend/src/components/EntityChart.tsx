 ...
 * EntityChart
 * 
 * Componente que busca dados históricos do InfluxDB e exibe em gráfico.
 * Mostra últimos 30 minutos de telemetria.
 */
...
export function EntityChart({ deviceId, entityId, entityName, unit }: EntityChartProps) {
  ...
  const fetchHistoricalData = async () => {
    try {
      ...
      // Buscar últimos 30 minutos usando a rota correta
      const response = await api.get(`/api/v1/telemetry/${deviceId}/${entityId}`, {
        params: {
          start: '-30m',  // Últimos 30 minutos
          window: '1m',   // Agregação de 1 minuto
        },
      });
  ...
  if (data.length === 0) {
    return (
      <div className="h-64 flex flex-col items-center justify-center text-gray-500">
        ...
        <p>Sem dados disponíveis nos últimos 30 minutos</p>
      </div>
    );
  }
